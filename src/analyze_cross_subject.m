...
% removing subject DM's data which was influenced by a bug in the phone app
...
elseif strcmp(os, 'win'),
    ...
    features_dir = 'C:\\Users\\sst485\\Dropbox\\Data\\FTC\\features\\archive\\';
    models_dir = 'C:\\Users\\sst485\\Dropbox\\Data\\FTC\\models\\';
    slsh = '\\';
else
    error('Unknown OS');
end
...
n_subj_train_range = 1:length(subjects)-1;
...
%             features_test_sit = [features_test_sit; features_sit{subj_test}{trial}];
%             features_test_walk = [features_test_walk; features_walk{subj_test}{trial}];
%             labels_test_sit = [labels_test_sit; repmat({'2'},size(features_sit{subj_test}{trial},1),1)];
%             labels_test_walk = [labels_test_walk; repmat({'1'},size(features_walk{subj_test}{trial},1),1)];
%         end
%         features_test = [features_test_sit; features_test_walk];
%         labels_test = [labels_test_sit; labels_test_walk];
        
        % testing only on normal activity data
        features_test = [features_sit{subj_test}{1}; features_walk{subj_test}{1}];
        labels_test = [repmat({'2'},size(features_sit{subj_test}{1},1),1); repmat({'1'},size(features_walk{subj_test}{1},1),1)];
        ...
        %% training and testing the baseline models
        ...
        %% training and testing the expert models
        
        accuracy_btstp_walk = zeros(n_btstp, 1);
        accuracy_btstp_sit = zeros(n_btstp, 1);
        accuracy_btstp = zeros(n_btstp, 1);
...
if write_results,
    ...
    save('accuracy_onnormal.mat', 'accuracy_expert', 'accuracy_baseline');
end
