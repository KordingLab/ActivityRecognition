...
for m = 1:length(runTypes)
    ...
    for k = 1:12
        ...
        clear checkStates
        for p = 1:length(groundTruth)
           checkStates{p} =  uniqueStates{groundTruth(p)};
        end
        [hmmConTemp acc] = createConfusionMatrix(checkStates,postPredictedStates);
        hmmCon = hmmCon + hmmConTemp;
    end
    
    %save hmm results
    filename = ['CV_HMM_' runType];
    save(filename,'hmmResults','svmCon');
