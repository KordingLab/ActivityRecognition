...
for j = testSubject:testSubject 
    ...
    yvec_nums = cell2vec(cellstr(trainClassifierData.activity)');
    testvec = zeros(size(yvec_nums));  
    [svmPredictedStates classifierFit P] = classify('svm',trainClassifierData.features,...
        cellstr(trainClassifierData.activity)',testvec,c,gamma);
    ...
    [prePredictedCodes, accuracy, P] = svmpredict(...
        cell2vec(cellstr(testClassifierData.activity)')',...
    ...
    groundTruth = zeros(length(testClassifierData.activity),1);
    for i = 1:length(testClassifierData.activity)
        groundTruth(i,1) = find(strcmp(testClassifierData.activity{i},uniqueStates));
    end
...
%     end
    ...
    [preCon,accuracyAll] = createConfusionMatrix(cellstr(testClassifierData.activity'), prePredictedStates');
