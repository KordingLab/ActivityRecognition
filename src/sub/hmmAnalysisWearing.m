...
for m = 1:length(runTypes)
    ...
    for k = 1:12
        ...
        elseif strcmp(runType,'Wearing')
            [A,T,R] = xlsread(transitionFile,'Sheet5');
        else
            %5x5 matrix
...
%         wearing = {'Bag','Belt','Hand','Pocket'};
        activity = {'Misc','Sit to Stand','Sitting','Stand to Sit','Standing','Walking'};
        activityAcc = zeros(length(activity),1);
        for w = 1:length(activity)
            %get indices for wearing condition
            tempInd = find(strcmp(activity{w},results{1,k}.testClassifierData.activity));
            ...
            svmActivityAcc(k,w) = numSVMRight / length(tempInd);
            activityAcc(w) = numRight / length(tempInd);
            activityVec(k,w) = activityAcc(w);
        end
        hmmResults{1,k}.activityAcc = activityAcc;
        hmmResults{1,k}.activityLabels = activity;
