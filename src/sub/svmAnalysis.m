...
for j = testSubject:testSubject 
    ...
    [prePredictedCodes, accuracy, P] = svmpredict(...
        cell2vec(cellstr(testClassifierData.states)')',...
    ...
    groundTruth = zeros(length(testClassifierData.states),1);
    for i = 1:length(testClassifierData.states)
        groundTruth(i,1) = find(strcmp(testClassifierData.states{i},uniqueStates));
    end
    ...
    [preCon,accuracyAll] = createConfusionMatrix(cellstr(testClassifierData.states'), prePredictedStates');
